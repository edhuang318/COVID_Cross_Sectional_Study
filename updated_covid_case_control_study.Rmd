---
title: "Updated COVID Case Control Study"
author: "Elijah"
date: '2023-10-04'
...
setwd("~/Documents/covid-19-time-series-metrics-by-county-and-state-orj0nv")
```
...
head(covid_cases_deaths_demographics$report_date)
tail(covid_cases_deaths_demographics$report_date)
```

Let's explore the data. I want to know what categories there are for race, sex, and age.

```{r}
covid_cases_deaths_demographics$report_date
unique(covid_cases_deaths_demographics$demographic_category) #"Age Group"      "Gender"         "Race Ethnicity"
unique(covid_cases_deaths_demographics$demographic_value)
```

So let's dive into age.

```{r}
age_0_to_17 <- covid_cases_deaths_demographics %>%
  filter(demographic_value == "0-17")
tail(age_0_to_17)
tail(age_0_to_17, 1) #checking to see that this is the last line
cases_age_0_to_17 <- age_0_to_17$total_cases[719]
age_18_to_49 <- covid_cases_deaths_demographics %>%
  filter(demographic_value == "18-49")
tail(age_18_to_49)
cases_age_18_to_49 <- age_18_to_49$total_cases[719]
age_50_to_64 <- covid_cases_deaths_demographics %>%
  filter(demographic_value == "50-64")
tail(age_50_to_64)
cases_age_50_to_64 <- age_50_to_64$total_cases[719]
age_65_up <- covid_cases_deaths_demographics %>%
  filter(demographic_value == "65+")
tail(age_65_up)
cases_age_65_up <- age_65_up$total_cases[719]
```
...
key <- c("1fd361849952fe6d87edfdc07962d8805c8e0903")
...
#Controls for 0-17
control_cases_ca_0_to_17 <- ca_0_to_17 - cases_age_0_to_17 
#Controls for 18-49
control_cases_18_to_49 <- ca_18_to_49 - cases_age_18_to_49	
epi.2by2(c(cases_age_18_to_49,control_cases_18_to_49,cases_age_0_to_17,control_cases_ca_0_to_17), method = "case.control", digits = 4, conf.level = 0.95, units = 100, interpret = TRUE, outcome = "as.columns")
```

Now I will compare the 0-17 stratum against the 50-64 stratum.
```{r}
#Controls for 50-64
control_cases_ca_50_to_64 <- ca_50_to_64 - cases_age_50_to_64
control_cases_ca_50_to_64 
epi.2by2(c(cases_age_50_to_64,control_cases_ca_50_to_64,cases_age_0_to_17,control_cases_ca_0_to_17), method = "case.control", digits = 4, conf.level = 0.95, units = 100, interpret = TRUE, outcome = "as.columns")
```
...
```{r}
#Controls for 65+
control_cases_ca_65_and_over <- ca_65_and_over - cases_age_65_up
control_cases_ca_65_and_over 
epi.2by2(c(cases_age_65_up,control_cases_ca_65_and_over,cases_age_0_to_17,control_cases_ca_0_to_17), method = "case.control", digits = 4, conf.level = 0.95, units = 100, interpret = TRUE, outcome = "as.columns")
```
...
tail(age_0_to_17)
tail(age_0_to_17, 1) #checking to see that this is the last line
deaths_age_0_to_17 <- age_0_to_17$deaths[696]
tail(age_18_to_49)
deaths_age_18_to_49 <- age_18_to_49$deaths[696]
tail(age_50_to_64)
deaths_age_50_to_64 <- age_50_to_64$deaths[696]
tail(age_65_up)
deaths_age_65_up <- age_65_up$deaths[696]
```
...
```{r}
epi.2by2(c(deaths_age_18_to_49,control_deaths_18_to_49,deaths_age_0_to_17,control_deaths_0_to_17), method = "case.control", digits = 4, conf.level = 0.95, units = 100, interpret = TRUE, outcome = "as.columns")
```

Now to compare deaths between age group 0-17 with age group 50-64.

```{r}
epi.2by2(c(deaths_age_50_to_64,control_deaths_50_to_64,deaths_age_0_to_17,control_deaths_0_to_17), method = "case.control", digits = 4, conf.level = 0.95, units = 100, interpret = TRUE, outcome = "as.columns")
```

Now to compare deaths between age group 0 to 17 and age group 65 and up.

```{r}
epi.2by2(c(deaths_age_65_up,control_deaths_65_up,deaths_age_0_to_17,control_deaths_0_to_17), method = "case.control", digits = 4, conf.level = 0.95, units = 100, interpret = TRUE, outcome = "as.columns")
```

Now we move on to comparing cases between races. Let's find out how many cases happen in each race.

```{r}
white <- covid_cases_deaths_demographics %>%
  filter(demographic_value == "White")
tail(white)
tail(white, 1) #checking to see that this is the last line
cases_white <- white$total_cases[705]
black <- covid_cases_deaths_demographics %>%
  filter(demographic_value == "Black")
tail(black)
tail(black, 1) 
cases_black <- black$total_cases[705]
american_indian <- covid_cases_deaths_demographics %>%
  filter(demographic_value == "American Indian or Alaska Native")
tail(american_indian)
tail(american_indian, 1) 
cases_american_indian <- american_indian$total_cases[705]
asian <- covid_cases_deaths_demographics %>%
  filter(demographic_value == "Asian")
tail(asian)
tail(asian, 1) 
cases_asian <- asian$total_cases[705]
hawaiian_pacific_islander <- covid_cases_deaths_demographics %>%
...
cases_hawaiian_pacific_islander <- hawaiian_pacific_islander$total_cases[705]
multi_race <- covid_cases_deaths_demographics %>%
  filter(demographic_value == "Multi-Race")
other <- covid_cases_deaths_demographics %>%
  filter(demographic_value == "Other")
tail(other)
tail(other, 1) 
cases_other <- other$total_cases[705]
tail(multi_race)
tail(multi_race, 1) 
cases_multi_race <- multi_race$total_cases[705]
hispanic_latino <- covid_cases_deaths_demographics %>%
  filter(demographic_value == "Latino")
tail(hispanic_latino)
tail(hispanic_latino, 1) 
cases_hispanic_latino <- hispanic_latino$total_cases[705]
```
...
```{r}
epi.2by2(c(cases_black,control_cases_black,cases_white,control_cases_white), method = "case.control", digits = 4, conf.level = 0.95, units = 100, interpret = TRUE, outcome = "as.columns")
```

Now I will compare cases for white population vs. Asian population.

```{r}
epi.2by2(c(cases_asian,control_cases_asian,cases_white,control_cases_white), method = "case.control", digits = 4, conf.level = 0.95, units = 100, interpret = TRUE, outcome = "as.columns")
```

Now to compare cases for white population vs. American Indian population.

```{r}
epi.2by2(c(cases_american_indian,control_cases_american_indian,cases_white,control_cases_white), method = "case.control", digits = 4, conf.level = 0.95, units = 100, interpret = TRUE, outcome = "as.columns")
```

Now I will compare white population vs. Hawaiian and Pacific Islander pouplation.

```{r}
epi.2by2(c(cases_hawaiian_pacific_islander,control_cases_hawaiian_pacific_islander,cases_white,control_cases_white), method = "case.control", digits = 4, conf.level = 0.95, units = 100, interpret = TRUE, outcome = "as.columns")
```

Now I will compare cases in white population vs. Other race.

```{r}
epi.2by2(c(cases_other,control_cases_other,cases_white,control_cases_white), method = "case.control", digits = 4, conf.level = 0.95, units = 100, interpret = TRUE, outcome = "as.columns")
```

Now I will compare cases in white population vs. Multi-race population.

```{r}
epi.2by2(c(cases_multi_race,control_cases_multi_race,cases_white,control_cases_white), method = "case.control", digits = 4, conf.level = 0.95, units = 100, interpret = TRUE, outcome = "as.columns")
```

Now I will compare cases in white population vs. Hispanic/Latino population.

```{r}
epi.2by2(c(cases_hispanic_latino,control_cases_hispanic_latino,cases_white,control_cases_white), method = "case.control", digits = 4, conf.level = 0.95, units = 100, interpret = TRUE, outcome = "as.columns")
```
...
deaths_white <- white$deaths[705]
tail(black, 1) 
deaths_black <- black$deaths[705]
tail(american_indian, 1) 
deaths_american_indian <- american_indian$deaths[705]
tail(asian, 1) 
deaths_asian <- asian$deaths[705]
tail(hawaiian_pacific_islander, 1) 
deaths_hawaiian_pacific_islander <- hawaiian_pacific_islander$deaths[705]
tail(other, 1) 
deaths_other <- other$deaths[705]
tail(multi_race, 1) 
deaths_multi_race <- multi_race$deaths[705]
tail(hispanic_latino, 1) 
deaths_hispanic_latino <- hispanic_latino$deaths[705]
```
...
```{r}
epi.2by2(c(deaths_black,control_deaths_black,deaths_white,control_deaths_white), method = "case.control", digits = 4, conf.level = 0.95, units = 100, interpret = TRUE, outcome = "as.columns")
```

Now I will compare deaths in white population vs. American Indian population.

```{r}
epi.2by2(c(deaths_american_indian,control_deaths_american_indian,deaths_white,control_deaths_white), method = "case.control", digits = 4, conf.level = 0.95, units = 100, interpret = TRUE, outcome = "as.columns")
```

Now I will compare deaths in white population vs. Asian population.

```{r}
epi.2by2(c(deaths_asian,control_deaths_asian,deaths_white,control_deaths_white), method = "case.control", digits = 4, conf.level = 0.95, units = 100, interpret = TRUE, outcome = "as.columns")
```

Now I will compare deaths in white population vs. Hawaiian/Pacific Islander population.

```{r}
epi.2by2(c(deaths_hawaiian_pacific_islander,control_deaths_hawaiian_pacific_islander,deaths_white,control_deaths_white), method = "case.control", digits = 4, conf.level = 0.95, units = 100, interpret = TRUE, outcome = "as.columns")
```

Now I will compare deaths in white population vs. Other race.

```{r}
epi.2by2(c(deaths_other,control_deaths_other,deaths_white,control_deaths_white), method = "case.control", digits = 4, conf.level = 0.95, units = 100, interpret = TRUE, outcome = "as.columns")
```

Now I will compare deaths in white population vs. Multi-race population.

```{r}
epi.2by2(c(deaths_multi_race,control_deaths_multi_race,deaths_white,control_deaths_white), method = "case.control", digits = 4, conf.level = 0.95, units = 100, interpret = TRUE, outcome = "as.columns")
```

Now I will compare deaths in white population vs. Hispanic/Latino population.

```{r}
epi.2by2(c(deaths_hispanic_latino,control_deaths_hispanic_latino,deaths_white,control_deaths_white), method = "case.control", digits = 4, conf.level = 0.95, units = 100, interpret = TRUE, outcome = "as.columns")
```
...
```{r}
epi.2by2(c(cases_male,control_cases_male,cases_female,control_cases_female), method = "case.control", digits = 4, conf.level = 0.95, units = 100, interpret = TRUE, outcome = "as.columns")
```
...
```{r}
tail(male, 1) #checking to see that this is the last line
deaths_male <- male$deaths[696]
tail(female, 1) #checking to see that this is the last line
deaths_female <- female$deaths[696]
```
...
```{r}
epi.2by2(c(deaths_male,control_deaths_male,deaths_female,control_deaths_female), method = "case.control", digits = 4, conf.level = 0.95, units = 100, interpret = TRUE, outcome = "as.columns")
```

And that's it! I was going to do different periods during the epidemic, but I think that's more like a cross-sectional. The next step is compilation of all the statistics into a table and making charts and visuals.
The most unfortunate thing and biggest drawback is that these cases and deaths aren't reported by each individual with information on other variables, like vaccination status, or each individual's age, sex, and race. I can't calculate Mantel-Haentzel adjusted odds for this study because of that.