...
I will isolate the Delta Wave data points and repeat the overall case control study.
...
#set working directory manually. either the COVID SIRD or the orj0nv files works for this period.
```
Read in the dataset.
```{r}
...
```
Now select only the Delta Wave dates. From my previous project we learned that the Delta Wave was roughly from July to September 2021.
```{r}
delta <- covid_cases_deaths_demographics %>%
  filter(Report_Date >="2021-07-01" & Report_Date <= "2021-9-30")
```
Now the rest of the file will be the same as before, just changing the variables. I just have to change the covid_cases_deaths_demographics to delta at all points after this.
...
```{r}
delta$report_date
unique(delta$demographic_category) #"Age Group"      "Gender"         "Race Ethnicity"
unique(delta$demographic_value)
```
...
cases_age_0_to_17 <- age_0_to_17$total_cases[79] - age_0_to_17$total_cases[1]
age_18_to_49 <- delta %>%
  filter(demographic_value == "18-49")
tail(age_18_to_49)
cases_age_18_to_49 <- age_18_to_49$total_cases[79] - age_18_to_49$total_cases[1]
age_50_to_64 <- delta %>%
  filter(demographic_value == "50-64")
tail(age_50_to_64)
cases_age_50_to_64 <- age_50_to_64$total_cases[79]-age_50_to_64$total_cases[1]
age_65_up <- delta %>%
  filter(demographic_value == "65+")
tail(age_65_up)
cases_age_65_up <- age_65_up$total_cases[79]-age_65_up$total_cases[1]
```
...
epi.2by2(c(cases_age_18_to_49,control_cases_18_to_49,cases_age_0_to_17,control_cases_0_to_17), method = "case.control", digits = 4, conf.level = 0.95, units = 100, interpret = TRUE, outcome = "as.columns")
```
...
epi.2by2(c(cases_age_50_to_64,control_cases_50_to_64,cases_age_0_to_17,control_cases_0_to_17), method = "case.control", digits = 4, conf.level = 0.95, units = 100, interpret = TRUE, outcome = "as.columns")
```
...
```{r}
#Controls for 65+
control_cases_65_and_over <- ca_65_and_over - cases_age_65_up
control_cases_65_and_over 
epi.2by2(c(cases_age_65_up,control_cases_65_and_over,cases_age_0_to_17,control_cases_ca_0_to_17), method = "case.control", digits = 4, conf.level = 0.95, units = 100, interpret = TRUE, outcome = "as.columns")
```
...
head(age_0_to_17)
tail(age_0_to_17, 1) #checking to see that this is the last line
deaths_age_0_to_17 <- age_0_to_17$deaths[79] - age_0_to_17$deaths[1]
tail(age_18_to_49)
head(age_18_to_49)
deaths_age_18_to_49 <- age_18_to_49$deaths[79] - age_18_to_49$deaths[1]
tail(age_50_to_64)
deaths_age_50_to_64 <- age_50_to_64$deaths[79] - age_50_to_64$deaths[1]
tail(age_65_up)
deaths_age_65_up <- age_65_up$deaths[79] - age_65_up$deaths[1]
```
...
head(white)
cases_white <- white$total_cases[79] - white$total_cases[1]
black <- delta %>%
  filter(demographic_value == "Black")
tail(black)
tail(black, 1) 
cases_black <- black$total_cases[79] - black$total_cases[1]
american_indian <- delta %>%
  filter(demographic_value == "American Indian or Alaska Native")
tail(american_indian)
tail(american_indian, 1) 
cases_american_indian <- american_indian$total_cases[79] - american_indian$total_cases[1]
asian <- delta %>%
  filter(demographic_value == "Asian")
tail(asian)
tail(asian, 1) 
cases_asian <- asian$total_cases[79] - asian$total_cases[1]
hawaiian_pacific_islander <- delta %>%
...
cases_hawaiian_pacific_islander <- hawaiian_pacific_islander$total_cases[79] - hawaiian_pacific_islander$total_cases[1]
multi_race <- delta %>%
...
tail(other)
tail(other, 1) 
cases_other <- other$total_cases[79] - other$total_cases[1]
tail(multi_race)
tail(multi_race, 1) 
cases_multi_race <- multi_race$total_cases[79] - multi_race$total_cases[1]
hispanic_latino <- delta %>%
  filter(demographic_value == "Latino")
tail(hispanic_latino)
tail(hispanic_latino, 1) 
cases_hispanic_latino <- hispanic_latino$total_cases[79] - hispanic_latino$total_cases[1]
```
...
```{r}
tail(white) #checking to see what the last line is
deaths_white <- white$deaths[79] - white$deaths[1]
tail(black, 1) 
deaths_black <- black$deaths[79] - black$deaths[1]
tail(american_indian, 1) 
deaths_american_indian <- american_indian$deaths[79] - american_indian$deaths[1]
tail(asian, 1) 
deaths_asian <- asian$deaths[79] - asian$deaths[1]
tail(hawaiian_pacific_islander, 1) 
deaths_hawaiian_pacific_islander <- hawaiian_pacific_islander$deaths[79] - hawaiian_pacific_islander$deaths[1]
tail(other, 1) 
deaths_other <- other$deaths[79] - other$deaths[1]
tail(multi_race, 1) 
deaths_multi_race <- multi_race$deaths[79] - other$deaths[1]
tail(hispanic_latino, 1) 
deaths_hispanic_latino <- hispanic_latino$deaths[79] - hispanic_latino$deaths[1]
```
...
cases_male <- male$total_cases[79] - male$total_cases[1]
female <- delta %>%
  filter(demographic_value == "Female")
tail(female)
tail(female, 1) #checking to see that this is the last line
cases_female <- female$total_cases[79] - female$total_cases[1]
```
...
```{r}
tail(male) #checking to see that this is the last line
deaths_male <- male$deaths[79] - male$deaths[1]
tail(female, 1) #checking to see that this is the last line
deaths_female <- female$deaths[79] - female$deaths[1]
```
