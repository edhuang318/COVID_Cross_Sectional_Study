---
title: "BA.4/BA.5 Case Control Analysis"
author: "Elijah Huang"
date: '2023-11-09'
...
```
I will isolate the BA.4/BA.5 Wave data points and repeat the overall case control study.
...
#set working directory manually. or the orj0nv files works for this period.
```
Read in the dataset.
```{r}
...
```
Now select only the BA.4/BA.5 dates. From my previous project we learned that the BA.4/BA.5 Wave was roughly from April to September 2022.
```{r}
ba_4_ba_5 <- covid_cases_deaths_demographics %>%
  filter(Report_Date >="2022-04-01" & Report_Date <= "2022-9-30")
```
Now the rest of the file will be the same as before, just changing the variables. I just have to change the covid_cases_deaths_demographics to ba_4_ba_5 at all points after this.


Let's explore the data. I want to know what categories there are for race, sex, and age.

```{r}
ba_4_ba_5$report_date
unique(ba_4_ba_5$demographic_category) #"Age Group"      "Gender"         "Race Ethnicity"
unique(ba_4_ba_5$demographic_value)
```

So let's dive into age.

```{r}
age_0_to_17 <- ba_4_ba_5 %>%
  filter(demographic_value == "0-17")
tail(age_0_to_17)
head(age_0_to_17)
tail(age_0_to_17, 1) #checking to see that this is the last line
cases_age_0_to_17 <- age_0_to_17$total_cases[48] - age_0_to_17$total_cases[1]
age_18_to_49 <- ba_4_ba_5 %>%
  filter(demographic_value == "18-49")
tail(age_18_to_49)
cases_age_18_to_49 <- age_18_to_49$total_cases[48] - age_18_to_49$total_cases[1]
age_50_to_64 <- ba_4_ba_5 %>%
  filter(demographic_value == "50-64")
tail(age_50_to_64)
cases_age_50_to_64 <- age_50_to_64$total_cases[48]-age_50_to_64$total_cases[1]
age_65_up <- ba_4_ba_5 %>%
  filter(demographic_value == "65+")
tail(age_65_up)
cases_age_65_up <- age_65_up$total_cases[48]-age_65_up$total_cases[1]
```
...
epi.2by2(c(cases_age_50_to_64,control_cases_50_to_64,cases_age_0_to_17,control_cases_ca_0_to_17), method = "cross.sectional", digits = 4, conf.level = 0.95, units = 100, interpret = TRUE, outcome = "as.columns")
```
...
epi.2by2(c(cases_age_65_up,control_cases_65_and_over,cases_age_0_to_17,control_cases_ca_0_to_17), method = "cross.sectional", digits = 4, conf.level = 0.95, units = 100, interpret = TRUE, outcome = "as.columns")
```

Now I also want to compare gender and race in case control studies. Either men or women can serve as the baseline exposure, but I think setting white race as baseline exposure is a good idea. I also want to do case control 2x2 tables for age, gender, and race specifically during the ba_4_ba_5 and Omicron Periods (and maybe a non-wave period too).
...
head(age_0_to_17)
tail(age_0_to_17, 1) #checking to see that this is the last line
deaths_age_0_to_17 <- age_0_to_17$deaths[48] - age_0_to_17$deaths[1]
tail(age_18_to_49)
head(age_18_to_49)
deaths_age_18_to_49 <- age_18_to_49$deaths[48] - age_18_to_49$deaths[1]
tail(age_50_to_64)
deaths_age_50_to_64 <- age_50_to_64$deaths[48] - age_50_to_64$deaths[1]
tail(age_65_up)
deaths_age_65_up <- age_65_up$deaths[48] - age_65_up$deaths[1]
```
...
head(white)
cases_white <- white$total_cases[48] - white$total_cases[1]
black <- ba_4_ba_5 %>%
  filter(demographic_value == "Black")
tail(black)
tail(black, 1) 
cases_black <- black$total_cases[48] - black$total_cases[1]
american_indian <- ba_4_ba_5 %>%
  filter(demographic_value == "American Indian or Alaska Native")
tail(american_indian)
tail(american_indian, 1) 
cases_american_indian <- american_indian$total_cases[48] - american_indian$total_cases[1]
asian <- ba_4_ba_5 %>%
  filter(demographic_value == "Asian")
tail(asian)
tail(asian, 1) 
cases_asian <- asian$total_cases[48] - asian$total_cases[1]
hawaiian_pacific_islander <- ba_4_ba_5 %>%
...
cases_hawaiian_pacific_islander <- hawaiian_pacific_islander$total_cases[48] - hawaiian_pacific_islander$total_cases[1]
multi_race <- ba_4_ba_5 %>%
  filter(demographic_value == "Multi-Race")
other <- ba_4_ba_5 %>%
  filter(demographic_value == "Other")
tail(other)
tail(other, 1) 
cases_other <- other$total_cases[48] - other$total_cases[1]
tail(multi_race)
tail(multi_race, 1) 
cases_multi_race <- multi_race$total_cases[48] - multi_race$total_cases[1]
hispanic_latino <- ba_4_ba_5 %>%
  filter(demographic_value == "Latino")
tail(hispanic_latino)
tail(hispanic_latino, 1) 
cases_hispanic_latino <- hispanic_latino$total_cases[48] - hispanic_latino$total_cases[1]
```
...
deaths_white <- white$deaths[48] - white$deaths[1]
tail(black, 1) 
deaths_black <- black$deaths[48] - black$deaths[1]
tail(american_indian, 1) 
deaths_american_indian <- american_indian$deaths[48] - american_indian$deaths[1]
tail(asian, 1) 
deaths_asian <- asian$deaths[48] - asian$deaths[1]
tail(hawaiian_pacific_islander, 1) 
deaths_hawaiian_pacific_islander <- hawaiian_pacific_islander$deaths[48] - hawaiian_pacific_islander$deaths[1]
tail(other, 1) 
deaths_other <- other$deaths[48] - other$deaths[1]
tail(multi_race, 1) 
deaths_multi_race <- multi_race$deaths[48] - other$deaths[1]
tail(hispanic_latino, 1) 
deaths_hispanic_latino <- hispanic_latino$deaths[48] - hispanic_latino$deaths[1]
tail(other)
head(other)
other_cases_plot <- ggplot(other, aes(x = Report_Date, y = total_cases)) +
  geom_point()
other_cases_plot
other_deaths_plot <- ggplot(other, aes(x = Report_Date, y = deaths)) +
  geom_point()
other_deaths_plot
hispanic_latino_deaths_plot <- ggplot(hispanic_latino, aes(x = Report_Date, y = deaths)) +
  geom_point()
hispanic_latino_deaths_plot
other_plot <- ggplot(other, aes(x = Report_Date)) +
  geom_point(data = other, aes(y = total_cases, color = "Total Cases")) +
  geom_point(data = other, aes(y = deaths, color = "Total Deaths")) +
  scale_y_continuous(labels = scales::comma) +
  scale_color_manual(name = "",
                     breaks = c("Total Cases", "Total Deaths"),
...
cases_male <- male$total_cases[48] - male$total_cases[1]
female <- ba_4_ba_5 %>%
  filter(demographic_value == "Female")
tail(female)
tail(female, 1) #checking to see that this is the last line
cases_female <- female$total_cases[48] - female$total_cases[1]
```
...
deaths_male <- male$deaths[48] - male$deaths[1]
tail(female, 1) #checking to see that this is the last line
deaths_female <- female$deaths[48] - female$deaths[1]
```
